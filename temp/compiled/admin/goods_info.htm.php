...
?>
        ...
        <table width="90%" id="detail-table" style="display:none">
			...
			<!--
				...
				1、2013 12 27 二次开发，让每一个布料都有一个描述的框。

				2、添加刷新的按钮，点击按钮的时候，ajax调取有多少的布料，然后根据不同的布料。

				   显示不同的描述框，并将描述框形成html放在这里。
			...
			<?php $_from = $this->_var['FCKeditor']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('j', 'i');if (count($_from)):
    foreach ($_from AS $this->_var['j'] => $this->_var['i']):
?>
            		<?php $_from = $this->_var['color_res']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('j1', 'i1');if (count($_from)):
    foreach ($_from AS $this->_var['j1'] => $this->_var['i1']):
?>
						<?php if ($this->_var['j'] == $this->_var['j1']): ?>
							color:<?php echo $this->_var['i1']['attr_value']; ?>
						<?php endif; ?>
					<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
...
/**
	...
	*an 二次开发 点击描述的按钮，按照布料的属性改变描述的个数（仿照getAttrList修改）

*/



	function getAttrList_changeDesc(goodsId)

	{

		Ajax.call('goods.php?act=get_attr_changeDesc', 'goods_id=' + goodsId, setAttr_changeDesc, "GET", "JSON");

	}

	

	function setAttr_changeDesc(result, text_result)

	{

		document.getElementById('detail-table').innerHTML = result.content;
