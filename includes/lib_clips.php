...
{
    $sql = 'SELECT g.goods_id,g.goods_img, g.goods_name, g.market_price, g.shop_price AS org_price, '.
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'g.promote_price, g.promote_start_date,g.promote_end_date, c.rec_id,color_id, c.is_attention' .
   ...
   foreach($res as $k=>$v)
    {
        ...
        {
            $promote_price = bargain_price($res[$k]['promote_price'], $res[$k]['promote_start_date'], $res[$k]['promote_end_date']);
        }
        else
        {
            $promote_price = 0;
        }
        $goods_list[$k]['rec_id']        = $res[$k]['rec_id'];
		
		$goods_list[$k]['color_id']      = $res[$k]['color_id'];
        $goods_list[$k]['is_attention']  = $res[$k]['is_attention'];
        $goods_list[$k]['goods_id']      = $res[$k]['goods_id'];
        $goods_list[$k]['goods_name']    = $res[$k]['goods_name'];
        $goods_list[$k]['market_price']  = price_format($res[$k]['market_price']);
        $goods_list[$k]['shop_price']    = price_format($res[$k]['shop_price']);
        $goods_list[$k]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
        $goods_list[$k]['url']           = build_uri('goods', array('gid'=>$res[$k]['goods_id']), $res[$k]['goods_name']);
		$goods_list[$k]['goods_img']    = $res[$k]['goods_img'];
		
		$sql_attr = 'select attr_value from '. $GLOBALS['ecs']->table('goods_attr').' where goods_attr_id = '.$res[$k]['color_id'].' and goods_id ='.$res[$k]['goods_id'].'';
		
		$color_name = $GLOBALS['db']->getOne($sql_attr); 
		
		$goods_attr_img_sql = 'select img_url from '. $GLOBALS['ecs']->table('goods_gallery').' where img_desc = "'.$color_name.'" and goods_id ='.$res[$k]['goods_id'].'';
		
		$goods_attr_img = $GLOBALS['db']->getOne($goods_attr_img_sql); 
		
		$goods_list[$k]['color_name'] =$color_name;
		$goods_list[$k]['color_img'] =$goods_attr_img;
...
//         $sql = "SELECT ua.consignee, ua.email, ua.tel, ua.mobile ".
        $sql = "SELECT ua.address_name, ua.email, ua.tel, ua.mobile ".
