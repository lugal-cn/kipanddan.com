...
function edit_address(action,id){
	var action = action;
	var id = id;
	
	$.ajax({
		type:'post',
		url:'flow.php?step=edit_address',
		data:{
			action:action,
			id:id,
		},
		success:function(msg){
			if(action =='shipping'){
				$('#addAddr_shipping').html(msg);	
			}else{
				$('#addAddr_billing').html(msg);	
			}
		}
	});
}
function make_defalut(action,id)
{
	...
	$.ajax({
		type:'post',
		url:'flow.php?step=make_defalut',
		data:{
			action:action,
			id:id,
		},
