...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'open_chart')
{
	
	echo "<img src='http://kipanddan.com/an_website/data/afficheimg/website size chart.jpg' height='890px'>";
	exit();
}
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'open_terms')
{
...
//-- anyongchao 添加的ajax的操作返回很多前台需要的问题

//-- 2013.12.09 15:15

if(!empty($_REQUEST) && $_REQUEST['act'] == 'return_change_color')

{

	include('includes/cls_json.php');   //调用json文件

    $json   = new JSON;					//实例化json

    $res    = array('err_msg' => '', 'result' => '');

	

	$goods_id = strip_tags(urldecode($_POST['goods_id']));

	$goods_id = json_str_iconv($_POST['goods_id']);

	

	

	$color_id = strip_tags(urldecode($_POST['color_id']));

	$color_id = json_str_iconv($_POST['color_id']);

	

	

	if(!empty($color_id) || !empty($goods_id))

	{

			$sql = 'SELECT * FROM'. $ecs->table('goods_gallery') .' WHERE goods_id = '. $goods_id .' order by img_id ASC';

			$image_gallery = $db->getAll($sql);
...
//-- ajax 同时加上shadowbox的操作，返回产品的描述的版块 

/*-------------------------------------------------------*/

	if(!empty($_REQUEST) && $_REQUEST['act'] == 'ajax_product_info')
		{
			...
			$sql3 = 'SELECT * FROM'. $ecs->table('goods_gallery') .' WHERE goods_id = '. $id .'';
			$color_img= $db->getAll($sql3);
			$img_arr =array();
			//产品描述
			$desc_arr = unserialize($desc);
			if($desc_arr == '' || empty($desc_arr)){
				echo'';
			}else{
...
//-- sizeCharset
	...
	if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'opensize')

	{

		$smarty->display('goods_size.dwt');
