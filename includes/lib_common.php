...
{
$sql = "SELECT article_id, title,open_type,cat_id,file_url FROM " .$GLOBALS['ecs']->table('article'). " WHERE cat_id = ".$cat_aid." and is_open = 1 order by article_id asc  LIMIT " . $cat_num;
...
function cat_list_an($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
    ...
    if ($level > 0)
    {
		...
		if ($re_type == true)
		{
			...
			foreach ($options AS $var)
			{
				...
				if($var['parent_id'] == 0){
					$str .='<li class="main"><a href="category.php?id='.$var['cat_id']. '">';
					$num =0;
					$wit = $var['has_children'] + 1;
					$new_html = '<li class="new"><a href="search.php?intro=new&category='.$var['cat_id']. '">NEW</a>';
				}else{
					$str .='<li ><a href="category.php?id='.$var['cat_id']. '">';
				
				}
				
				//$select .= ($selected == $var['cat_id']) ? "selected='ture'" : '';
				
				$str .= htmlspecialchars(addslashes($var['cat_name']), ENT_QUOTES) . '</a></li>';
			...
			//$str .= '<li class="new"><a href="search.php?intro=new&category='.$var['cat_id'].'">NEW</a></li>';
		...
		else
		{
			foreach ($options AS $key => $value)
			{
				...
				$str .='<li><a href="category.php?id='.$value['cat_id'].'" class="main">'.$value['cat_name'].'</a></li>';
			}
...
function price_format($price, $change_price = false)
