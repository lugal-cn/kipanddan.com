...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        /* 图片列表 */
		/*an 修改20131226 让图片按照id 排序*/
        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]' order by img_id ASC";
	...
	*/
	$desc_arr =Array();
		...
		$sql = "SELECT * FROM " .$ecs->table('goods_attr'). " WHERE goods_id = '$_REQUEST[goods_id]' AND attr_id = '1'";
		
		$res = $db->getAll($sql);
		$goods_desc_array = json_decode($goods['goods_desc']);
		
		$sql2 = "SELECT goods_desc FROM " .$ecs->table('goods'). " WHERE goods_id = '$_REQUEST[goods_id]'";
		
		$res2 = $db->getAll($sql2);
		
		$xl_array = unserialize($res2[0]['goods_desc']);
		
		//unserialize($xl_array)
		
		foreach($res as $res_k=>$res_v){
			if($res == '' || empty($res_v)){
				$desc_arr[] = create_html_editor('goods_desc_'.$res_v['goods_attr_id'],'');
			} else {
					if(empty($xl_array) || $xl_array  == ''){
						$desc_arr[] = create_html_editor('goods_desc_'.$res_v['goods_attr_id'],'');
						
					}else{
						foreach($xl_array as $xl_k=>$xl_v){
						
							if(! empty($xl_v)){
								/*查询一下goods_desc形成一个数组*/
								if($res_v['goods_attr_id'] == $xl_v[0]){
									$desc_arr[] = create_html_editor('goods_desc_'.$res_v['goods_attr_id'], $xl_v[1]);
// 									$desc_arr[] = $xl_v[1];
								}
							}else{
								$desc_arr[] = create_html_editor('goods_desc_'.$res_v['goods_attr_id'],'');	
	...
	$smarty->assign('color_res',$res);
	$smarty->assign('FCKeditor', $desc_arr);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	//----an 二次开发 2013.12.30
	//-----将产品描述修改一下,将传过来的desc 组成一个数组，然后序列化，将他们按照color_id：描述储存
		$desc_array_xulie =array();
		foreach($_POST as $k=>$v){
			$var=explode("_",$k);
			if($var[0] == 'goods' && $var[1] == 'desc'){
				$desc_array_xulie[] =array($var[2],$v);
			}
		}
		//echo '<pre>';
		//var_dump($_POST);
		
		//var_dump($desc_array_xulie);
		$xl_array = serialize($desc_array_xulie);
		//print_r(unserialize($xl_array));
		//echo '</pre>';
		//exit();
		
		//print_r(unserialize(serialize($desc_array_xulie)));
        ...
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '".$xl_array."', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '".$xl_array."', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral')";
        }
    }
    else
    {
...
/*-------------------------------------------------------*/
//-- an 二次开发 ，将得到商品描述的模块的html
//-- 根据布料分配html
/*-------------------------------------------------------*/
elseif($_REQUEST['act'] == 'get_attr_changeDesc')
{
	check_authz_json('goods_manage');//调用goods_manage的json函数
	 
	$goods_id   = empty($_GET['goods_id']) ? 0 : intval($_GET['goods_id']);
	
	//admin 下的lib_goods.php中的函数
	//像下面这样去lib_goods.php组这个页面
	//这个应用an在本页面查询数据库并组合html
	//$content    = build_attr_html($goods_id);
	$content ='';
