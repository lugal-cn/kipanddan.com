...

{
...

    {
        ...
        // 更新：添加到购物车(这个函数在lib_order.php里面,作用是更新或者添加购物车)
...

        {
...

            {
...

                {
                    ...
                    ecs_header("Location: flow.php?step=
                    \n");
...

{
	...
	//根据这个step是get还是post得到是显示还是添加的操作
...

    {
...

        {
			...
			$sql = "SELECT  address_id , address_billing_id FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id = '".$_SESSION['user_id']."'";

			$defalut_address = $GLOBALS['db']->GetRow($sql);

			$smarty->assign('defalut_address',$defalut_address);

			

            $consignee_list      = get_consignee_list($_SESSION['user_id']);

			$consignee_list_billing = get_consignee_list_billing($_SESSION['user_id']);
           ...
           // {
			...
			$smarty->assign('consignee_list_num',count($consignee_list));

			$smarty->assign('consignee_list_billing_num',count($consignee_list_billing));
...

        {
			...
			//$_SESSION['flow_consignee']这个变量是session中的地址
...

            {
			 ...
			 if (isset($_SESSION['flow_consignee_billing'])){

                $consignee_list_billing = array($_SESSION['flow_consignee_billing']);
...

            {
                ...
                $consignee_list_billing[] = array('country' => $_CFG['shop_country']);
		...
		$smarty->assign('consignee_list_billing',$consignee_list_billing);

			

		//$consignee_list 这个是所有的收货人的列表
...

    {
		...
		if($_POST['address_classes'] =='billing'){

		

			$consignee_billing = array(

				'address_id'    => empty($_POST['address_id'])   ? 0  :   intval($_POST['address_id']),

				'firstname'     => empty($_POST['b_firstname'])  ? '' :   compile_str(trim($_POST['b_firstname'])),

				'surname'       => empty($_POST['b_surname'])    ? '' :   compile_str(trim($_POST['b_surname'])),

				'address'       => empty($_POST['b_address'])    ? '' :   compile_str($_POST['b_address']),

				'city'          => empty($_POST['b_city'])       ? '' :   compile_str(trim($_POST['b_city'])),

				'state'         => empty($_POST['b_state'])      ? '' :   compile_str(trim($_POST['b_state'])),

				'zipcode'       => empty($_POST['b_zipcode'])    ? '' :   compile_str(make_semiangle(trim($_POST['b_zipcode']))),

				'country'       => empty($_POST['b_country'])    ? '' :   intval($_POST['b_country']),

				'email'         => empty($_POST['b_email'])      ? '' :   compile_str(trim($_POST['b_email'])),

				'tel'           => empty($_POST['b_tel'])        ? '' :   compile_str(make_semiangle(trim($_POST['b_tel']))),
			 ...
			 $consignee = array(
				...
				'surname'       => empty($_POST['surname'])      ? '' :   compile_str(trim($_POST['surname'])),

				'address'       => empty($_POST['address'])      ? '' :   compile_str($_POST['address']),

				'city'          => empty($_POST['city'])         ? '' :   compile_str(trim($_POST['city'])),

				'state'         => empty($_POST['state'])        ? '' :   compile_str(trim($_POST['state'])),

				'zipcode'       => empty($_POST['zipcode'])      ? '' :   compile_str(make_semiangle(trim($_POST['zipcode']))),

				'country'       => empty($_POST['country'])      ? '' :   intval($_POST['country']),

				'email'         => empty($_POST['email'])        ? '' :   compile_str(trim($_POST['email'])),
...

        {
			...
			$consignee_billing['user_id'] = $_SESSION['user_id'];

			//lib_transaction.php 保存用户的收货人的信息
			
			if($_POST['address_classes'] =='billing'){
				
				 save_consignee_billing($consignee_billing, true);

			}else{
				if($_POST['same_above'] == 'on'){
					save_consignee_billing($consignee, true);
				}
        ...
        /* 保存到session */
			...
			if($_POST['address_classes'] =='billing'){
				 ...
				 $_SESSION['flow_consignee_billing'] = stripslashes_deep($consignee_billing);

			}else{
				if($_POST['same_above'] == 'on'){
					 $_SESSION['flow_consignee_billing'] = stripslashes_deep($consignee);
				}
...
else if($_REQUEST['step'] == 'make_defalut')

{
	...
	if(make_defalut($action ,$id)){

		echo 'This address has been made your default address.';

	

	}else{

		echo 'Change in default address unsuccessful, please try again.';
...
elseif($_REQUEST['step'] == 'user_add_address')
{
	$action =$_POST['action'];
		$country_list=get_regions();
		$str ='';
			$str .='<div class="blank2pxLine"></div><br>';
			if($action == 'shipping'){
				$str .='<table width="80%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#ffffff">';
					$str .='<tbody>';
						$str .='<tr>';
							$str .='<td style="height:60px;">shipping address:</td>';
							$str .='<td></td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>First name</td>';
							$str .='<td>';
								$str .='<input id="consignee_1" type="text" name="firstname">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Surname</td>';
							$str .='<td>';
								$str .='<input id="consignee_2" type="text" name="surname">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Address</td>';
							$str .='<td>';
								$str .='<input id="address" type="text" name="address">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Town/City</td>';
							$str .='<td>';
								$str .='<input id="city" type="text" name="city">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>State</td>';
							$str .='<td>';
								$str .='<input id="state" type="text" name="state">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Post code</td>';
							$str .='<td>';
								$str .='<input id="zipcode" type="text" name="zipcode">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Country</td>';
							$str .='<td>';
								$str .='<select id="selCountries" name="country">';
										$str .='<option value="0" selected>Select Country</option>';
						...
						$str .='<tr>';
							$str .='<td>Email</td>';
							$str .='<td>';
								$str .='<input id="email" type="text" name="email">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Phone</td>';
							$str .='<td>';
								$str .='<input id="tel" type="text" name="tel">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td colspan="2">';
								$str .='<input type="button" value="submit" onclick="check_shipping()">';
							$str .='</td>';
						$str .='</tr>';
					$str .='</tbody>';
					$str .='</table>';
							$str .='<input type="hidden" name="act" value="act_edit_address" />';
                            $str .='<input name="address_id" type="hidden" value="" />';
                            $str .='<input name="address_classes" type="hidden" value="shipping_address" />';

			}else if($action == 'billing'){
				$str .='<table width="80%" align="center" border="0" cellpadding="5" cellspacing="1" bgcolor="#ffffff">';
					$str .='<tbody>';
						$str .='<tr>';
							$str .='<td style="height:60px;">billing address:</td>';
							$str .='<td></td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>First name</td>';
							$str .='<td>';
								$str .='<input id="b_consignee_1" type="text" name="b_firstname">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Surname</td>';
							$str .='<td>';
								$str .='<input id="b_consignee_2" type="text" name="b_surname">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Address</td>';
							$str .='<td>';
								$str .='<input id="b_address" type="text" name="b_address">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Town/City</td>';
							$str .='<td>';
								$str .='<input id="b_city" type="text" name="b_city">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>State</td>';
							$str .='<td>';
								$str .='<input id="b_state" type="text" name="b_state">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Post code</td>';
							$str .='<td>';
								$str .='<input id="b_zipcode" type="text" name="b_zipcode">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Country</td>';
							$str .='<td>';
								$str .='<select id="b_selCountries" name="b_country">';
										$str .='<option value="0" selected>Select Country</option>';
											foreach($country_list as $k=>$v){
												$str .='<option value="'.$v['region_id'].'">'.$v['region_name'].'</option>';
											}
						...
						$str .='<tr>';
							$str .='<td>Email</td>';
							$str .='<td>';
								$str .='<input id="b_email" type="text" name="b_email">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Phone</td>';
							$str .='<td>';
								$str .='<input id="b_tel" type="text" name="b_tel">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td colspan="2">';
								$str .='<input type="button" value="submit" onclick="check_billing()">';
							$str .='</td>';
						$str .='</tr>';
					$str .='</tbody>';
					$str .='</table>';
				$str .='<input type="hidden" name="act" value="act_edit_address" />';
                            $str .='<input name="address_id" type="hidden" value="" />';
                            $str .='<input name="address_classes" type="hidden" value="billing_address" />';
			}
		$str .='</div>';
		echo $str;	
	exit;
...
else if($_REQUEST['step'] == 'add_address')

{
		...
		$str ='';
			...
			if($action == 'shipping'){
				...
				$str .='<input type="hidden" name="step" value="consignee" />';

				$str .='<input type="hidden" name="act" value="checkout" />';

						$str .='<input type="hidden" name="address_classes" value="shipping" />';
				...
				$str .='<input type="hidden" name="act" value="checkout" />';
						...
						$str .='<input type="hidden" name="address_classes" value="billing" />';
...
else if($_REQUEST['step'] == 'user_edit_address')
{
	include_once('includes/lib_transaction.php');
	$action =$_POST['action'];
	$id =$_POST['id'];
	...
	if($action == 'shipping'){
			...
			foreach($consignee_list as $k=>$v){
				if($consignee_list[$k]['address_id'] == $id){
					$rel_shipping = $consignee_list[$k];
				}
			}
			...
			$consignee_list_billing = get_consignee_list_billing($_SESSION['user_id']);
			foreach($consignee_list_billing as $k=>$v){
				if($consignee_list_billing[$k]['address_id'] == $id){
					$rel_billing = $consignee_list_billing[$k];
				}
			}
		} 
		$str ='';
			if($action == 'shipping'){
				$str .='<table width="70%" border="0" cellpadding="5" cellspacing="1" bgcolor="#ffffff">';
					$str .='<tbody>';
						
						$str .='<tr>';
							$str .='<td>First name</td>';
							$str .='<td>';
								$str .='<input id="consignee_1" type="text" name="firstname" value="'.$rel_shipping['firstname'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Surname</td>';
							$str .='<td>';
								$str .='<input id="consignee_2" type="text" name="surname" value="'.$rel_shipping['surname'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Address</td>';
							$str .='<td>';
								$str .='<input id="address" type="text" name="address" value="'.$rel_shipping['address'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Town/City</td>';
							$str .='<td>';
								$str .='<input id="city" type="text" name="city" value="'.$rel_shipping['city'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>State</td>';
							$str .='<td>';
								$str .='<input id="state" type="text" name="state" value="'.$rel_shipping['state'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Post code</td>';
							$str .='<td>';
								$str .='<input id="zipcode" type="text" name="zipcode" value="'.$rel_shipping['zipcode'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Country</td>';
							$str .='<td>';
								$str .='<select id="selCountries" name="country">';
										$str .='<option value="0">Select Country</option>';
											foreach($country_list as $k=>$v){
												if($v['region_id'] == $rel_shipping['country']){
						...
						$str .='<tr>';
							$str .='<td>Email</td>';
							$str .='<td>';
								$str .='<input id="email" type="text" name="email" value="'.$rel_shipping['email'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Phone</td>';
							$str .='<td>';
								$str .='<input id="tel" type="text" name="tel" value="'.$rel_shipping['tel'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td colspan="2">';
								$str .='<input type="button" value="update shipping address" onclick="check_shipping()">';
							$str .='</td>';
						$str .='</tr>';
					$str .='</tbody>';
					$str .='</table>';
				...
				$str .='<input type="hidden" value="act_edit_address" name="act">';
				$str .='<input type="hidden" value="'.$id.'" name="address_id">';
				$str .='<input type="hidden" value="shipping_address" name="address_classes">';
				...
				$str .='<table width="70%" border="0" cellpadding="5" cellspacing="1" bgcolor="#ffffff">';
					$str .='<tbody>';
						...
						$str .='<tr>';
							$str .='<td>First name</td>';
							$str .='<td>';
								$str .='<input id="b_consignee_1" type="text" name="b_firstname" value="'.$rel_billing['firstname'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Surname</td>';
							$str .='<td>';
								$str .='<input id="b_consignee_2" type="text" name="b_surname" value="'.$rel_billing['surname'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Address</td>';
							$str .='<td>';
								$str .='<input id="b_address" type="text" name="b_address" value="'.$rel_billing['address'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Town/City</td>';
							$str .='<td>';
								$str .='<input id="b_city" type="text" name="b_city" value="'.$rel_billing['city'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>State</td>';
							$str .='<td>';
								$str .='<input id="b_state" type="text" name="b_state" value="'.$rel_billing['state'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Post code</td>';
							$str .='<td>';
								$str .='<input id="b_zipcode" type="text" name="b_zipcode" value="'.$rel_billing['zipcode'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Country</td>';
							$str .='<td>';
								$str .='<select id="b_selCountries" name="b_country">';
										$str .='<option value="0">Select Country</option>';
											foreach($country_list as $k=>$v){
												if($v['region_id'] == $rel_billing['country']){
													$str .='<option value="'.$v['region_id'].'" selected>'.$v['region_name'].'</option>';
												}else{
													$str .='<option value="'.$v['region_id'].'">'.$v['region_name'].'</option>';
												}
						...
						$str .='<tr>';
							$str .='<td>Email</td>';
							$str .='<td>';
								$str .='<input id="b_email" type="text" name="b_email" value="'.$rel_billing['email'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td>Phone</td>';
							$str .='<td>';
								$str .='<input id="b_tel" type="text" name="b_tel" value="'.$rel_billing['tel'].'">';
							$str .='</td>';
						$str .='</tr>';
						$str .='<tr>';
							$str .='<td colspan="2">';
								$str .='<input type="button" value="update billing address" onclick="check_billing()">';
							$str .='</td>';
						$str .='</tr>';
					$str .='</tbody>';
					$str .='</table>';
				...
				$str .='<input type="hidden" value="act_edit_address" name="act">';
				$str .='<input type="hidden" value="'.$id.'" name="address_id">';
				$str .='<input type="hidden" value="billing_address" name="address_classes">';
			}
		$str .='</div>';
		echo $str;
	exit;
...
else if($_REQUEST['step'] == 'edit_address')

{
...

    {
		...
		if($action == 'shipping'){
			...
			$rel_shipping = $consignee_list;

		}else{

			$consignee_list_billing = $_SESSION['flow_consignee_billing'];

			$rel_billing = $consignee_list_billing;
		}
	}
	$str ='';
					...
					$str .='<tbody>';
						...
						$str .='<tr>';
							...
							$str .='<td colspan="2">';
								...
								$str .='<input type="submit" value="update shipping address">';
						...
						$str .='</tr>';
							...
							$str .='<input type="hidden" name="act" value="checkout" />';
							$str .='<input type="hidden" name="address_classes" value="shipping" />';
							$str .='<input name="address_id" type="hidden" value="'.$rel_shipping['address_id'].'" />';
					$str .='</tbody>';
					...
					$str .='<tbody>';
						...
						$str .='<tr>';
							...
							$str .='<td colspan="2">';
								...
								$str .='<input type="submit" value="update billing address">';
						...
						$str .='</tr>';
							...
							$str .='<input type="hidden" name="act" value="checkout" />';
							$str .='<input type="hidden" name="address_classes" value="billing" />';
							$str .='<input name="address_id" type="hidden" value="'.$rel_billing['address_id'].'" />';
...

{
	...
	//该函数在include/lib_order.php

    $consignee = get_consignee($_SESSION['user_id']);

	$consignee_billing = get_consignee_billing($_SESSION['user_id']);
	...
	if (!check_consignee_info_billing($consignee_billing, $flow_type))
	...
	$_SESSION['flow_consignee_billing'] = $consignee_billing;

	

    $smarty->assign('consignee', $consignee);

	$smarty->assign('consignee_billing', $consignee_billing);
    ...
    $cart_goods = cart_goods($flow_type); // 取得商品列表，计算合计
		...
		foreach($cart_goods as $k=>&$v){
			...
			$dd = explode('size',$v['goods_attr']);

			$color = trim(substr($dd[0],6,-1));

			$size  = trim(substr($dd[1],1,-1));

			$sql="SELECT thumb_url FROM " . $ecs->table('goods_gallery') . " WHERE img_desc = '$color' AND goods_id = '".$v['goods_id']."'";
			...
			$cart_goods[$k]['an_color'] = $color;

			$cart_goods[$k]['an_size'] = $size;

			

			//查出来的是产品颜色的简写

			$sql2 = "SELECT attr_price FROM " . $ecs->table('goods_attr') . " WHERE attr_value = '$color' AND goods_id = '".$v['goods_id']."'";

			$attr_jian = $db->getOne($sql2);

			

			$cart_goods[$k]['an_color_jian'] = $attr_jian;
...

{
    ...
    show_message('Update successful','Go back to cart', 'flow.php',false);
...

{
	...
	foreach($cart_goods['goods_list'] as $k=>&$v){
		...
		$cart_goods['goods_list'][$k]['an_color'] = $color;

		$cart_goods['goods_list'][$k]['an_size'] = $size;
		...
		$cart_goods['goods_list'][$k]['an_color_jian'] = $attr_jian;
