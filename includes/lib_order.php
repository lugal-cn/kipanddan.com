...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
			...
			// 购买商品是否超过5件，如果超过，赋值为0
            $count_number_sql = 'SELECT sum(goods_number) FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE  `session_id` = '" . SESS_ID. "' and `is_shipping` = 0";
            $count_number = $GLOBALS['db']->getOne($count_number_sql);

			if($count_number > 4){
				$total['shipping_fee']=0;
			}elseif($shipping_count == 0 AND $weight_price['free_shipping'] == 1){
				$total['shipping_fee']=0;
			}else{
    ...
    $total['shipping_fee_formated']    = price_format($total['shipping_fee'], false);    //运费
	
    $total['shipping_insure_formated'] = price_format($total['shipping_insure'], false); //报价，不用管
...
function get_consignee_billing($user_id)
{
    ...
    {
        /* 如果存在session，则直接返回session中的收货人信息 */
        return $_SESSION['flow_consignee_billing'];
    }
    else
    {
        /* 如果不存在，则取得用户的默认收货人信息 */
        ...
        if ($user_id > 0)
        {
            /* 取默认地址 */
            $sql = "SELECT ua.*".
                    " FROM " . $GLOBALS['ecs']->table('user_billing_address') . "AS ua, ".$GLOBALS['ecs']->table('users').' AS u '.
                    " WHERE u.user_id='$user_id' AND ua.address_id = u.address_billing_id";
...
function check_consignee_info($consignee, $flow_type)
{
    ...
    { 
        ...
        $res = !empty($consignee['firstname']) &&
			!empty($consignee['surname']) &&
            !empty($consignee['country']) &&
            !empty($consignee['address']) &&
			!empty($consignee['state']) &&
			!empty($consignee['city']) &&
			!empty($consignee['zipcode']) &&
...
function check_consignee_info_billing($consignee_billing, $flow_type)
{
	
    if (exist_real_goods(0, $flow_type))
    { 
        /* 如果存在实体商品 */
        $res = !empty($consignee_billing['firstname']) &&
			!empty($consignee_billing['surname']) &&
            !empty($consignee_billing['country']) &&
            !empty($consignee_billing['address']) &&
			!empty($consignee_billing['state']) &&
			!empty($consignee_billing['city']) &&
			!empty($consignee_billing['zipcode']) &&
			!empty($consignee_billing['email']) &&
            !empty($consignee_billing['tel']);

        if ($res)
        {
            if (empty($consignee_billing['province']))
            {
                /* 没有设置省份，检查当前国家下面有没有设置省份 */
                $pro = get_regions(1, $consignee_billing['country']);
                $res = empty($pro);
            }
            elseif (empty($consignee_billing['city']))
            {
                /* 没有设置城市，检查当前省下面有没有城市 */
                $city = get_regions(2, $consignee_billing['province']);
                $res = empty($city);
            }
            elseif (empty($consignee_billing['district']))
            {
                $dist = get_regions(3, $consignee_billing['city']);
                $res = empty($dist);
            }
        }
