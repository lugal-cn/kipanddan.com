	...
	*/
	 function login($username, $password, $remember = null)
    {
        ...
        if ($this->check_user($username, $password) > 0)
        {
            if ($this->need_sync)
            {
                $this->sync($email,$password);
            }
            $this->set_session2($username);
            $this->set_cookie2($username, $remember);
...
*/
	function check_user($username, $password = null)
    {
        ...
        if ($password === null)
        {
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE " . $this->field_username . "='" . $username . "'";
        ...
        else
        {
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE " . $this->field_username . "='" . $username . "' AND " . $this->field_pass . " ='" . $this->compile_password(array('password'=>$password)) . "'";
     ...
     * @return void
     */
    function set_cookie($email='', $remember= null )
    {
        if (empty($email))
        {
            /* 摧毁cookie */
        ...
        elseif ($remember)
        {
            ...
            setcookie("ECS[username]", $email, $time, $this->cookie_path, $this->cookie_domain);
            $sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE email='$email' LIMIT 1";
	...
	function set_cookie2($username='', $remember= null )
    {
        if (empty($username))
        {
            /* 摧毁cookie */
    ...
    function set_session ($email='')
    {
        ...
        {
            $sql = "SELECT user_id,user_name, password, email FROM " . $GLOBALS['ecs']->table('users') . " WHERE email='$email' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
	  ...
	  function set_session2 ($username='')
    {
        if (empty($username))
        {
            $GLOBALS['sess']->destroy_session();
