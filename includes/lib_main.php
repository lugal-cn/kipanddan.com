...
function update_user_info()
{
    ...
    /* 修改登出不清空购物车 */
    // @link http://blog.sina.com.cn/s/blog_6ec2ae900101okpd.html
    $sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET". " session_id = '" .SESS_ID . "'".
    		" WHERE user_id = '" .$_SESSION['user_id'] . "'"; $GLOBALS['db']->query($sql);
}
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('category_list', cat_list_an(0, 0, true,  2, false)); //自己修改 cat_list_an 和cat_list去lib_common.php找
    $smarty->assign('catalog_list',  cat_list_an(0, 0, false, 1, false));//自己修改
