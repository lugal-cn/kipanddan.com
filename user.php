...
$not_login_arr =
array('log_reg','login','register','act_register','act_login','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('log_reg','register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
...
if (in_array($action, $ui_arr))
{
    ...
    $smarty->assign('page_title', 'My account'); // 页面标题
...
if ($action == 'default')
{
	...
	$address_billing_id  = $db->getOne("SELECT address_billing_id FROM " .$ecs->table('users'). " WHERE user_id='$user_id'");
	$smarty->assign('address_id',      $address_id);
	$smarty->assign('address_billing_id',     $address_billing_id);
...
// 			show_message('Registration successful!', array('Continue','Go to My Account'), array('index.php','user.php'), 'info',false);
        ...
        {
            $err->show($_LANG['sign_up'], 'user.php');
        }
    }
}
...
/* 显示新添加的注册登录的页面,仿照下面的做*/
elseif($action == 'log_reg')
{
	 if (empty($back_act))
    {
	...
	//广告
	$ad_sql='select * from '. $ecs->table("ad").' where position_id=2';
	$login_ad=$db->getAll($ad_sql);
	
	
	$smarty->assign('login_ad', $login_ad[0]['ad_code']);
	$smarty->assign('back_act', $back_act);
 	$smarty->display('user_passport.dwt');
}
...
elseif ($action == 'signin')
{
    ...
    {
        update_user_info();  //更新用户信息（更新用户SESSION,COOKIE及登录时间、登录次数。）
        recalculate_price(); // 重新计算购物车中的商品价格（重新计算购物车中的商品价格：目的是当用户登录时享受会员价格，当用户退出登录时不享受会员价格）
...
elseif ($action == 'address_list')
{
   ...
   // }
    ...
    $smarty->assign('del_address',          $address_id);
	$smarty->assign('del_address_billing',          $address_billing_id);
...
elseif ($action == 'act_edit_address')
{
	...
	$address_classes = isset($_POST['address_classes'])?compile_str(trim($_POST['address_classes'])):'';
	if($address_classes =='shipping_address'){
	
			$address = array(
			
				'user_id'    => $user_id,
				'address_id' => intval($_POST['address_id']),//根据有没有address_id来判断是添加还是修改
				'country'    => isset($_POST['country'])?intval($_POST['country']):0,//国家的id
				'email'      => isset($_POST['email'])?compile_str(trim($_POST['email'])):'',//邮箱的
				'firstname'  => isset($_POST['firstname'])?compile_str(trim($_POST['firstname'])):'',
				'surname'    => isset($_POST['surname'])?compile_str(trim($_POST['surname'])):'',
				'address'    => isset($_POST['address'])?compile_str(trim($_POST['address'])):'',
				'city'       => isset($_POST['city'])?compile_str(trim($_POST['city'])):'',
				'state'      => isset($_POST['state'])?compile_str(trim($_POST['state'])):'',
				'zipcode'    => isset($_POST['zipcode'])?compile_str(trim($_POST['zipcode'])):'',
				'tel'        => isset($_POST['tel'])?compile_str(trim($_POST['tel'])):'',
			);
			//该函数在include中的lib_transaction.php文件中的，
			//添加或更新指定用户收货地址
			
				if($_POST['same_above'] == 'on'){
					update_billing_address($address);
				}
			
			if (update_address($address))
			{
				show_message('Your address has been successfully saved.','Return to Address', 'user.php?act=address_list');
			}
			
	}else if($address_classes =='billing_address'){
	
		$address = array(
		
			'user_id'    => $user_id,
			'address_id' => intval($_POST['address_id']),//根据有没有address_id来判断是添加还是修改
			'country'    => isset($_POST['b_country'])?intval($_POST['b_country']):0,//国家的id
			'email'      => isset($_POST['b_email'])?compile_str(trim($_POST['b_email'])):'',//邮箱的
			'firstname'  => isset($_POST['b_firstname'])?compile_str(trim($_POST['b_firstname'])):'',
			'surname'    => isset($_POST['b_surname'])?compile_str(trim($_POST['b_surname'])):'',
			'address'    => isset($_POST['b_address'])?compile_str(trim($_POST['b_address'])):'',
			'city'       => isset($_POST['b_city'])?compile_str(trim($_POST['b_city'])):'',
			'state'      => isset($_POST['b_state'])?compile_str(trim($_POST['b_state'])):'',
			'zipcode'    => isset($_POST['b_zipcode'])?compile_str(trim($_POST['b_zipcode'])):'',
			'tel'        => isset($_POST['b_tel'])?compile_str(trim($_POST['b_tel'])):'',
        );
...
/* 删除收货地址 */
elseif ($action == 'drop_consignee_billing')
{
    include_once('includes/lib_transaction.php');

    $consignee_id = intval($_GET['id']);

    if (drop_consignee_billing($consignee_id))
    {
        ecs_header("Location: user.php?act=address_list\n");
        exit;
    }
...
/*得到收藏的商品的info*/
elseif($action == 'ajax_product_info')
{
	$id = $_GET['id'];
	//得到的是这个收藏的东西的查询
	$sql = "SELECT * FROM " .$ecs->table('collect_goods')." WHERE user_id='$user_id' AND goods_id='$id'";
	$dd = $db->getAll($sql);
	$color_id = $dd[0][color_id];
	...
	$html .='<html>';
	$html .='<head>';
		$html .='<link href="themes/default/css/select_size.css" rel="stylesheet" type="text/css" />';
		$html .='<script type="text/javascript" src="js/jquery-1.10.2.min.js"></script>';
		$html .='<script type="text/javascript" src="js/shadowbox.js"></script>';
		$html .='<script type="text/javascript" src="js/select_size.js"></script>';
	
		$html .='<script type="text/javascript" src="js/transport.js"></script>';
		$html .='<script type="text/javascript" src="js/utils.js"></script>';
	$html .='</head>';
	
	$html .='<body>';
		$html .='<div class="size_list">';
			$html .='<table>';
			$html .='<tr><td style="background-color:#ffffff" colspan="3">Select Size</td></tr>';
				$html .='<tr>';
				foreach($size as $k=>$v){
				
						$html .='<td class="size_select" onclick="select_size_switch('.$v['id'].')">'.$v['label'].'</td>';
				}
					
				$html .='</tr>';
			$html .='<table>';
		$html .='</div>';
		$html .='<form id="ECS_FORMBUY" name="ECS_FORMBUY" method="post" action="javascript:addToCart('.$color_id.')">';
		$html .='<input type="hidden" id="color_option" name="color" value="'.$color_id.'">';
		$html .='<input type="hidden" id="size_option" name="size" value="">';
		$html .='<input type="hidden" id="number" name="number" value="1">';
		$html .='<div class="button_area">';
		$html .='<span class="button_1" onclick="addToCart('.$id.');">Add to basket</span>';
		$html .='</div>';
		$html .='<div class="notice_info"></div>';
		$html .='</form>';
	$html .='</body>';
	$html .='</html>';
	echo $html;
...
elseif ($action == 'collect')
{
    ...
    if (!isset($_SESSION['user_id']) || $_SESSION['user_id'] == 0)
    {
		...
		$result['message'] = 'Please login or register to add to wishlist.';
        die($json->encode($result));
    }
    else
    {
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('collect_goods') .
            " WHERE user_id='$_SESSION[user_id]' AND goods_id = '$goods_id' AND color_id = '$color_id'";
        if ($GLOBALS['db']->GetOne($sql) > 0)
        {
            $result['error'] = 1;
           // $result['message'] = $GLOBALS['_LANG']['collect_existed'];
			$result['message'] = 'Item already on your wishlist.';
            die($json->encode($result));
        }
        else
        {
            $time = gmtime();
            $sql = "INSERT INTO " .$GLOBALS['ecs']->table('collect_goods'). " (user_id, goods_id,color_id,add_time)" .
                    "VALUES ('$_SESSION[user_id]', '$goods_id','$color_id', '$time')";

            if ($GLOBALS['db']->query($sql) === false)
            {
                $result['error'] = 1;
                //$result['message'] = $GLOBALS['db']->errorMsg();
				$result['message'] = 'Sorry, there was an error.';
            ...
            else
            {
               ...
               // $result['message'] = $GLOBALS['_LANG']['collect_success'];
				$result['message'] = 'Item has been added to your wishlist.';
                die($json->encode($result));
            }
        }
    }
